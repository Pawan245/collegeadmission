...
public partial class registraionfeepayment : System.Web.UI.Page
        ...
        try
        {
            ...
            else
            {
                ...
                if (fg.Trim().Equals("4"))
                {
                   ...
                   String hyt=     dt.inserttransactiontable(studentid , HttpContext.Current.Session["pricer"].ToString());


                     //   HttpContext.Current.Response.Write(hyt);


                        amount.Value = HttpContext.Current.Session["pricer"].ToString();
        ...
        if (tandc.Checked == true)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(Request.Form["hash"])) // generating hash value
                {
                    ...
                    else
                    {
                        ...
                        foreach (string hash_var in hashVarsSeq)
                        {
                            ...
                            {
                                hash_string = hash_string + HttpContext.Current.Session["namec"].ToString();
                                hash_string = hash_string + '|';
                            }

                            else if (hash_var == "email")
                            {
                                hash_string = hash_string + HttpContext.Current.Session["emailct"].ToString();
                                hash_string = hash_string + '|';
                            }
                            ...
                            else if (hash_var == "amount")
                            {
                                ...
                                String amo = HttpContext.Current.Session["pricer"].ToString();
                ...
                if (!string.IsNullOrEmpty(hash1))
                {
                    ...
                    String pricet = HttpContext.Current.Session["pricer"].ToString();


                    string AmountForm = Convert.ToDecimal(pricet.Trim()).ToString("g29");// eliminating trailing zeros
                                                                                         //  amount.Text = AmountForm;
                    data.Add("amount", AmountForm);
                    data.Add("firstname", HttpContext.Current.Session["namec"].ToString());
                    data.Add("email", HttpContext.Current.Session["emailct"].ToString());
                    data.Add("phone", HttpContext.Current.Session["mobilec"].ToString());
                    data.Add("productinfo", "info");
                    data.Add("surl", "http://localhost:52936/regfeepaidstatus");
                    data.Add("furl", "http://localhost:52936/regfeepaidstatus");
